  ...
  <div class="mian">
    ...
    <div class="content">
        ...
        <table width="100%" border="0" cellspacing="1" cellpadding="0"  style="color:#FFF; font-size:13px; font-family:Tahoma, Geneva, sans-serif; border:  thin #999 solid; ">
		 ...
		 ?>
          <tr height="25">
            ...
            <a class="group1" href="_pic_file_cn/<?=$arr['AR_CNP_PIC_NAME']?>" title="<?=$arr['AR_CNP_PIC_NAME']?>">
            ...
            <a href="clear_temp.php?id=<?=md5('del_pic_cn')?>&gitem=<?=$arr['AR_CNP_ITEM']?>&namep=<?=$arr['AR_CNP_PIC_NAME']?>&item=<?=$_SESSION['id_item']?>"  onClick="return confirm('คุณแน่ใจหรือไม่')">
      ...
      <fieldset style="width:96%; margin-left:11px; margin-bottom:10px;">
        ...
        <form method="post" action="add_edit_pic_cn.php?id_up=<?=$_SESSION['id_cn']?>&amp;id_key=<?=md5('add_pic')?>" name="02" enctype="multipart/form-data">
    ...
    <!---  <a href="clear_temp.php?id=<?=md5('del_pic_all')?>&item=<?=$_SESSION['id_item']?>&itemp=<?=($i-1)?>" class="clear_list"  onClick="return confirm('คุณแน่ใจหรือไม่')"></a>--->
       ...
       <a href="clear_temp.php?id=<?=md5('del_pic_calcle')?>&item=<?=$_SESSION['id_item']?>&itemp=<?=($i-1)?>" class="back" onClick="return confirm('คุณแน่ใจหรือไม่')"></a> <BR />
      <BR />
      <?PHP
